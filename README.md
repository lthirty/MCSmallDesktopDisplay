...
## 4. v1.4加气象预警 增量更新记录（2026-04-10）

1. 开机 Logo 更新
- 开机页面 Logo 更换为客户素材 `横标.jpg`
- 显示方式调整为保持原图比例并居中，且不遮挡进度条

2. 配置提示中文化
- `Connect to Config` 提示改为中文“进入配置页面”
- 采用图片资源渲染中文，避免字库缺字导致方框显示
- 配置地址保持为 `http://sd3.local`

3. 运行画面右下角图片更新
- 红框区域图片改为 `竖标-1-1.jpg`
- 对应资源头文件为 `img/indoor_bubu.h`
