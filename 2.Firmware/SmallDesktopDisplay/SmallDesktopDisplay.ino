 ...
 *            V1.5.0  2026.04.12
 *                    1) Web常驻管理页与WiFi配网页字段统一为中文，并按“基础设置/高级设置”分组。
 *                    2) 增加城市快速选择下拉菜单（同时保留手动输入城市代码）。
 *                    3) 屏幕运行界面新增当前版本号常显。
 *            V1.6.0  2026.04.12
 *                    1) 去掉运行界面版本号常显，避免遮挡主画面信息。
 *                    2) 修复开机/等待配置阶段底部提示语显示异常（恢复连接提示与版本提示）。
 *                    3) 增加Web菜单：可选择开机Logo图片与运行页右下角图片（并保存到EEPROM）。
 * 
 *            V1.6.1  2026.04.12
 *                    1) Web管理页改为整页宽度显示（去掉窄栏限制）。
 *                    2) 修复城市下拉保存后不生效：保存后立即触发天气刷新。
 *                    3) 修复天气更新时间异常值（如255）启动时回落为10分钟。
 *            V1.6.2  2026.04.12
 *                    1) 城市选择改为“城市名输入+自动推荐”，提升易用性。
 *                    2) 选择城市名后自动同步城市代码并参与保存。
 *                    3) 服务端增加城市名兜底映射，防止前端脚本失败导致保存无效。
 *            V1.6.3  2026.04.12
 *                    1) Web新增手机相册上传图片（JPG/JPEG）并写入设备。
 *                    2) 开机Logo与运行页右下角图片支持自定义上传图并保存选择。
 *                    3) 上传图片按显示区域自动缩放适配显示。
 *            V1.6.5  2026.04.12
 *                    1) 运行页右下角优先显示LittleFS里的/run.jpg，避免回退太空人图。
 *                    2) 右下角图片整体上移，避免遮挡IP地址。
 *            V1.6.6  2026.04.12
 *                    1) Web页面UI放大字体与控件，提升手机端可读性。
 *                    2) 图片上传增加阶段与进度提示（上传/改模/适配/设置）。
 *                    3) 运行页右下角仅显示IP地址本体，去掉前缀。
  *            V1.6.7  2026.04.12
 *                    1) Web字体与控件按需求进一步放大（约3倍）。
 *                    2) 修复右下角IP显示裁切与湿度区域被遮挡问题，重排右下角布局。
 *                    3) 图片上传增加串口诊断与JPEG可解码校验，避免“上传成功但不生效”。
 *            V1.6.8  2026.04.12
 *                    1) 右下角图片下移（不遮挡IP显示）。
 *                    2) 修复左侧湿度文本被IP区域覆盖：缩窄并右移IP绘制区域。
 *                    3) 去掉Web页面底部“By WCY”。
 *            V1.6.9  2026.04.12
 *                    1) 放宽JPG上传校验，兼容更多手机相机导出编码。
 *                    2) 上传后仍保留串口日志，便于定位异常。
 *                    3) 释放COM口占用后可正常打开串口监视器。
 *            V1.6.14  2026.04.12
 *                    1) Web上传提示中运行图分辨率建议由≤420x420调整为≤350x350。
 *                    2) 运行页右下角图片绘制前增加并集区域清屏，彻底去除旧坐标残留图层。
 *                    3) 统一默认图回退坐标到新位置，避免新旧坐标叠加遮挡。
 *            V1.6.15  2026.04.12
 ...
 * *****************************************************************/
#define Version  "MuchCode-V1.6.30"
/* *****************************************************************
 *  库文件、头文件
 * *****************************************************************/
...
int Boot_Logo_Flag = 0; //开机Logo选择标志位
int Run_Img_Flag = 0;   //运行页右下角图片选择标志位
uint8_t indoorAvatarDirty = 1; //右下角头像重绘标志位，避免周期性闪烁
uint8_t bootLogoDrawn = 0; //开机Logo只绘制一次，避免进度更新时反复刷屏
File webUploadFile;
...
int BootLogo_addr = 5; //5 开机Logo选择
int RunImg_addr = 6;   //6 运行页右下角图片选择
...
bool drawCustomJpegInBox(const char* path, int x, int y, int boxW, int boxH)
{
  ...
  // 某些手机JPG无法读取尺寸，直接尝试绘制，不在上传阶段判死刑。
  TJpgDec.setJpgScale(1);
...
bool validateUploadedJpeg(const char* path, String& err)
{
  ...
  if(sz < 16)
  {
    err = "上传失败：文件内容异常（可能存储空间不足）";
    return false;
  }

  if(!(b1 == 0xFF && b2 == 0xD8))
  {
    // 不再阻断保存，部分手机导出文件头可能异常，运行时继续尝试解码。
    Serial.printf("JPEG头异常但保留文件: %s, b1=%u, b2=%u, size=%u\r\n", path, b1, b2, (unsigned)sz);
  }
  ...
  {
    Serial.printf("JPEG尺寸探测失败但保留文件: %s, size=%u, jr=%d\r\n", path, (unsigned)sz, (int)jr);
    return true;
  }
...
void drawBootLogo()
{
  switch (Boot_Logo_Flag)
  {
    case 2:
      ...
      break;
    case 1:
      TJpgDec.drawJpg(20, 44, misaka, sizeof(misaka));
      break;
    default:
      TJpgDec.drawJpg(20, 44, boot_logo_bubu, sizeof(boot_logo_bubu));
      break;
...
{
  clk.setColorDepth(8);
  clk.createSprite(200, 56);
  clk.fillSprite(0x0000);
  clk.setTextDatum(CC_DATUM);
  clk.setTextColor(TFT_GREEN, 0x0000);
  clk.drawString("Connecting to WiFi......",100,16,2);
  clk.setTextColor(TFT_WHITE, 0x0000);
  clk.drawString(Version,100,40,2);
  clk.pushSprite(20,170);
  clk.deleteSprite();
}
...
void drawRuntimeCornerImage()
{
  const int clearX = 170;
  const int clearY = 157;
  const int clearW = 60;
  const int clearH = 60;

  // 目标图固定为60x60像素，并锚定在右下角，避免遮挡其它信息。
  const int imgW = 60;
  const int imgH = 60;
  const int imgX = 170;
  const int imgY = 157;
  ...
  // 如果存在用户上传图，优先显示，避免回退到默认图。
  if(drawCustomJpegInBox("/run.jpg", imgX, imgY, imgW, imgH)) return;

  switch (Run_Img_Flag)
  {
    case 2:
      if(!drawCustomJpegInBox("/run.jpg", imgX, imgY, imgW, imgH))
        TJpgDec.drawJpg(imgX, imgY, indoor_bubu, sizeof(indoor_bubu));
      break;
    case 1:
      TJpgDec.drawJpg(imgX, imgY, misaka, sizeof(misaka));
      break;
    default:
      TJpgDec.drawJpg(imgX, imgY, indoor_bubu, sizeof(indoor_bubu));
      break;
...
{
  String msg;
  String bootUploadStatusText = server.hasArg("boot_upload_ok") ? "阶段：上传和配置成功" : "阶段：待上传";
  String runUploadStatusText = server.hasArg("run_upload_ok") ? "阶段：上传和配置成功" : "阶段：待上传";
  String bootUploadProgressVal = server.hasArg("boot_upload_ok") ? "100" : "0";
  String runUploadProgressVal = server.hasArg("run_upload_ok") ? "100" : "0";
  int web_cc,web_setro,web_lcdbl,web_upt,web_dhten,web_cc_preset,web_bootimg,web_runimg,web_cc_name;
  String web_city_name;

  if (server.hasArg("web_ccode") || server.hasArg("web_city_name") || server.hasArg("web_city_preset") || server.hasArg("web_bl") || \
      server.hasArg("web_upwe_t") || server.hasArg("web_DHT11_en") || server.hasArg("web_set_rotation") || server.hasArg("web_boot_img") || server.hasArg("web_run_img")) 
  {
    web_cc    = server.arg("web_ccode").toInt();
    web_cc_preset = server.arg("web_city_preset").toInt();
    ...
    web_bootimg = server.arg("web_boot_img").toInt();
    web_runimg = server.arg("web_run_img").toInt();

    if(web_cc_preset >= 101000000 && web_cc_preset <= 102000000)
      web_cc = web_cc_preset;
    if(web_cc_name >= 101000000 && web_cc_name <= 102000000)
      web_cc = web_cc_name;
    ...
    if(web_bootimg >= 0 && web_bootimg <= 2)
    {
      Boot_Logo_Flag = web_bootimg;
      EEPROM.write(BootLogo_addr, Boot_Logo_Flag);
      EEPROM.commit();
      delay(5);
      Serial.print("Boot Logo Flag:");
      Serial.println(Boot_Logo_Flag);
    }

    if(web_runimg >= 0 && web_runimg <= 2)
    {
      Run_Img_Flag = web_runimg;
      EEPROM.write(RunImg_addr, Run_Img_Flag);
      EEPROM.commit();
      delay(5);
      indoorAvatarDirty = 1;
      Serial.print("Run Image Flag:");
      Serial.println(Run_Img_Flag);
    }
  }

  //网页界面代码段
  String rot0 = (LCD_Rotation == 0) ? " selected" : "";
  String rot1 = (LCD_Rotation == 1) ? " selected" : "";
  String rot2 = (LCD_Rotation == 2) ? " selected" : "";
  String rot3 = (LCD_Rotation == 3) ? " selected" : "";
  String dhtChecked = (DHT_img_flag != 0) ? " checked" : "";
  
  String boot0 = (Boot_Logo_Flag == 0) ? " selected" : "";
  String boot1 = (Boot_Logo_Flag == 1) ? " selected" : "";
  String boot2 = (Boot_Logo_Flag == 2) ? " selected" : "";
  String run0 = (Run_Img_Flag == 0) ? " selected" : "";
  String run1 = (Run_Img_Flag == 1) ? " selected" : "";
  String run2 = (Run_Img_Flag == 2) ? " selected" : "";
  String content = "<html><meta charset='utf-8'><style>";
  content += "html,body{background:#0f172a;color:#e2e8f0;font-size:39px;font-family:Arial,sans-serif;margin:0;line-height:1.6;}";
  content += ".wrap{width:100%;max-width:none;margin:0;padding:16px;box-sizing:border-box;}";
  content += "fieldset{border:1px solid #334155;border-radius:10px;padding:14px 14px;margin:12px 0;}";
  content += "legend{color:#7dd3fc;padding:0 6px;font-size:39px;font-weight:700;}label{font-size:36px;}input,select{width:100%;box-sizing:border-box;padding:18px;margin:12px 0;background:#111827;color:#fff;border:1px solid #334155;border-radius:10px;font-size:39px;}";
  content += ".radio-row{margin:10px 0 14px 0;} .radio-row label{display:block;margin:8px 0;font-size:36px;}";
  content += ".btn{width:100%;padding:18px;border:0;border-radius:10px;background:#06b6d4;color:#fff;font-weight:bold;font-size:39px;}";
  content += ".ver{font-size:30px;color:#93c5fd;margin-bottom:10px;}h2{font-size:48px;margin:8px 0 12px 0;} .switch{display:flex;align-items:center;gap:14px;margin:14px 0;} .switch input{width:72px;height:42px;}";
  content += "</style>";
  content += "<body><div class='wrap'><form action='/' method='POST'>";
  content += "<h2>智能台历管理页</h2>";
  content += "<div class='ver'>当前版本：" + String(Version) + "</div>";
  content += "<fieldset><legend>基础设置</legend>";
  content += "城市名输入（自动推荐）<input id='web_city_name' list='city_name_list' type='text' name='web_city_name' placeholder='例如：长沙/上海' oninput='syncCityCode()' onchange='syncCityCode()'>";
  content += "<datalist id='city_name_list'>";
  content += "<option value='北京'></option><option value='上海'></option><option value='广州'></option><option value='深圳'></option>";
  content += "<option value='杭州'></option><option value='南京'></option><option value='武汉'></option><option value='西安'></option>";
  content += "<option value='重庆'></option><option value='成都'></option><option value='长沙'></option><option value='珠海'></option>";
  content += "</datalist>";
  content += "城市代码（可手动输入）<input id='web_ccode' type='text' name='web_ccode' value='" + cityCode + "' placeholder='例如：101282005'>";
  content += "<script>function syncCityCode(){var m={'北京':'101010100','上海':'101020100','广州':'101280101','深圳':'101280601','杭州':'101210101','南京':'101190101','武汉':'101200101','西安':'101110101','重庆':'101040100','成都':'101270101','长沙':'101250101','珠海':'101280701'};var n=document.getElementById('web_city_name');var c=document.getElementById('web_ccode');if(!n||!c)return;var key=(n.value||'').trim();if(m[key]){c.value=m[key];}}</script>";
  content += "屏幕亮度（1-100）<input type='text' name='web_bl' value='" + String(LCD_BL_PWM) + "'>";
  content += "开机Logo图片<select name='web_boot_img'><option value='0'" + boot0 + ">客户横标</option><option value='1'" + boot1 + ">Misaka图</option><option value='2'" + boot2 + ">自定义上传图</option></select>";
  content += "运行页右下角图片<select name='web_run_img'><option value='0'" + run0 + ">布布头像</option><option value='1'" + run1 + ">Misaka图</option><option value='2'" + run2 + ">自定义上传图</option></select>";
  content += "屏幕方向<select name='web_set_rotation'><option value='0'" + rot0 + ">USB朝下</option><option value='1'" + rot1 + ">USB朝右</option><option value='2'" + rot2 + ">USB朝上</option><option value='3'" + rot3 + ">USB朝左</option></select></fieldset>";
  content += "<fieldset><legend>高级设置</legend>";
  content += "天气更新时间（分钟，1-60）<input type='text' name='web_upwe_t' value='" + String(updateweater_time) + "'>";
  #if DHT_EN
  content += "DHT传感器开关<div class='switch'><input type='checkbox' name='web_DHT11_en' value='1'" + dhtChecked + "><span>开启</span></div>";
  #endif
  content += "</fieldset>";
...
void handleBootUpload()
{
  ...
  else if(upload.status == UPLOAD_FILE_WRITE)
  {
    if(webUploadFile)
    {
      ...
      Serial.printf("[UPLOAD][BOOT] WRITE: +%u, wrote=%u, total=%u, saved=%u\r\n", upload.currentSize, (unsigned)wr, webUploadBytes, webWrittenBytes);
    }
  }
  else if(upload.status == UPLOAD_FILE_END)
  {
    if(webUploadFile) webUploadFile.close();
    Serial.printf("[UPLOAD][BOOT] END: total=%u, saved=%u\r\n", webUploadBytes, webWrittenBytes);
    if(webUploadError.length()==0 && webWrittenBytes < webUploadBytes){ webUploadError = "开机图上传失败：图片过大或存储空间不足"; }

    if(webUploadError.length() == 0)
    {
      String verifyErr = "";
      if(validateUploadedJpeg("/boot.jpg", verifyErr))
      {
        Boot_Logo_Flag = 2;
        EEPROM.write(BootLogo_addr, Boot_Logo_Flag);
        EEPROM.commit();
        delay(5);
        Serial.println("[UPLOAD][BOOT] APPLY OK");
      }
      else
      {
        webUploadError = verifyErr;
        Serial.printf("[UPLOAD][BOOT] VERIFY FAIL: %s\r\n", webUploadError.c_str());
      }
    }
  }
  ...
  {
    if(webUploadFile) webUploadFile.close();
    webUploadError = "开机图上传失败：上传中断";
    Serial.println("[UPLOAD][BOOT] ABORTED");
  }
}
...
void handleRunUpload()
{
  ...
  else if(upload.status == UPLOAD_FILE_WRITE)
  {
    if(webUploadFile)
    {
      ...
      webUploadBytes += upload.currentSize;
      webWrittenBytes += (uint32_t)wr;
      Serial.printf("[UPLOAD][RUN] WRITE: +%u, wrote=%u, total=%u, saved=%u\r\n", upload.currentSize, (unsigned)wr, webUploadBytes, webWrittenBytes);
    }
  }
  else if(upload.status == UPLOAD_FILE_END)
  {
    if(webUploadFile) webUploadFile.close();
    Serial.printf("[UPLOAD][RUN] END: total=%u, saved=%u\r\n", webUploadBytes, webWrittenBytes);
    if(webUploadError.length()==0 && webWrittenBytes < webUploadBytes){ webUploadError = "运行页图片上传失败：图片过大或存储空间不足"; }

    if(webUploadError.length() == 0)
    {
      String verifyErr = "";
      if(validateUploadedJpeg("/run.jpg", verifyErr))
      {
        Run_Img_Flag = 2;
        EEPROM.write(RunImg_addr, Run_Img_Flag);
        EEPROM.commit();
        delay(5);
        indoorAvatarDirty = 1;
        Serial.println("[UPLOAD][RUN] APPLY OK");
      }
      else
      {
        webUploadError = verifyErr;
        Serial.printf("[UPLOAD][RUN] VERIFY FAIL: %s\r\n", webUploadError.c_str());
      }
    }
  }
  else if(upload.status == UPLOAD_FILE_ABORTED)
  {
    if(webUploadFile) webUploadFile.close();
    webUploadError = "运行页图片上传失败：上传中断";
    Serial.println("[UPLOAD][RUN] ABORTED");
  }
}
...
void Web_win()
{
  ...
  clk.setTextColor(TFT_WHITE, 0x0000); 
  clk.drawString("SSID: AutoConnectAP",100,28,2);
  clk.pushSprite(20,132);  //窗口位置
...
void Webconfig()
{
  ...
  delay(3000);
  // wm.resetSettings(); // 禁用自动清空WiFi，避免每次都触发WiFi Connect Fail
  ...
  #if DHT_EN
  WiFiManagerParameter  custom_DHT11_en("DHT11_en","DHT传感器开关(0/1)","0",1);
  #endif
  WiFiManagerParameter  custom_weatertime("WeaterUpdateTime","天气更新时间(分钟)","10",3);
  WiFiManagerParameter  custom_cc("CityCode","城市代码(支持手填)","101282005",9);
  WiFiManagerParameter  custom_boot_logo("BootLogo","开机Logo(0客户横标/1Misaka)","0",1);
  WiFiManagerParameter  custom_run_img("RunImage","运行页右下角图(0布布/1Misaka)","0",1);
  const char* p_adv_title = "<div style='margin:8px 0 4px 0;padding:6px;background:#0f172a;color:#fff;border-radius:4px;'><b>高级设置</b></div>";
  WiFiManagerParameter  custom_adv_title(p_adv_title);
  WiFiManagerParameter  p_lineBreak_notext("<p></p>");

  // wm.addParameter(&p_lineBreak_notext);
  // wm.addParameter(&custom_field);
  wm.addParameter(&p_lineBreak_notext);
  wm.addParameter(&custom_basic_title);
  wm.addParameter(&custom_city_select);
  ...
  wm.addParameter(&p_lineBreak_notext);
  wm.addParameter(&custom_adv_title);
  wm.addParameter(&custom_weatertime);
  wm.addParameter(&p_lineBreak_notext);
  wm.addParameter(&custom_boot_logo);
  wm.addParameter(&p_lineBreak_notext);
  wm.addParameter(&custom_run_img);
  #if DHT_EN
  wm.addParameter(&p_lineBreak_notext);
...
void saveParamCallback(){
  ...
  bootLogoSel = getParam("BootLogo").toInt();
  if(bootLogoSel >= 0 && bootLogoSel <= 2)
  {
    Boot_Logo_Flag = bootLogoSel;
    EEPROM.write(BootLogo_addr, Boot_Logo_Flag);
    EEPROM.commit();
    delay(5);
    Serial.print("Boot Logo Flag:");
  ...
  runImageSel = getParam("RunImage").toInt();
  if(runImageSel >= 0 && runImageSel <= 2)
  {
    Run_Img_Flag = runImageSel;
    EEPROM.write(RunImg_addr, Run_Img_Flag);
    EEPROM.commit();
    delay(5);
    indoorAvatarDirty = 1;
...
void setup()
{
 ...
 //从eeprom读取背光亮度设置
  ...
  //从eeprom读取开机Logo与运行页右下角图片选择
  Boot_Logo_Flag = EEPROM.read(BootLogo_addr);
  if(Boot_Logo_Flag > 2) Boot_Logo_Flag = 0;
  Run_Img_Flag = EEPROM.read(RunImg_addr);
  if(Run_Img_Flag > 2) Run_Img_Flag = 0;
  ...
  if(strlen(wificonf.stassid) > 0) WiFi.begin(wificonf.stassid, wificonf.stapsw);
  else WiFi.begin();
  ...
  while (WiFi.status() != WL_CONNECTED) 
  {
    ...
    if(loadNum>=194)
    {
      ...
      {
        WiFi.begin(); // 兜底：尝试使用SDK内保存的WiFi凭据
...
#if imgAst_EN
  if(DHT_img_flag == 0 && indoorAvatarDirty)
  {
    tft.fillRect(170, 160, 70, 62, bgColor);
    drawRuntimeCornerImage();
    indoorAvatarDirty = 0;
  }
#endif
...
void drawRuntimeIpTag()
{
  String ipTxt = "--";
  if(WiFi.status() == WL_CONNECTED)
    ipTxt = WiFi.localIP().toString();
  clk.setColorDepth(8);
  clk.createSprite(84, 14);
  clk.fillSprite(bgColor);
  clk.setTextDatum(TR_DATUM);
  clk.setTextColor(0xBDF7, bgColor);
  clk.drawString(ipTxt, 82, 8, 1);
  clk.pushSprite(156, 214);
  clk.deleteSprite();
}
